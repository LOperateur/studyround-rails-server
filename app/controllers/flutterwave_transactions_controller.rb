class FlutterwaveTransactionsController < TransactionsController
  require 'flutterwave_sdk'

  def initiate
    transaction_ref = generate_transaction_ref("flw")
  ...
  def process_transaction
    ...
    unless card.is_flutterwave_card?
    ...
    tx_ref = generate_transaction_ref("flw")
  ...
  def build_trx_success_response(data, save_card = true)
    ...
    begin
      ...
      transaction.purchase_item_id = data.dig('meta', 'item_id') || process_transaction_params[:item_id] # Meta not passed during token charge
      transaction.purchase_item_type = data.dig('meta', 'item_type') || process_transaction_params[:item_type] # Meta not passed during token charge
      ...
      raise Errors::BaseError.new(message: "Unable to process transaction, please contact customer care", status: 400)
    end
  ...
  def save_card(card)
    ...
    create_or_update_card(new_card)
  end
end
